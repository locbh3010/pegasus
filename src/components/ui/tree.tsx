...
function Tree({ indent = 20, tree, className, toggleIconType = 'chevron', ...props }: TreeProps) {
  let containerProps = {}
  if (tree && typeof tree === 'object' && tree !== null && 'getContainerProps' in tree) {
    const treeWithMethod = tree as { getContainerProps?: () => Record<string, unknown> }
    if (typeof treeWithMethod.getContainerProps === 'function') {
      containerProps = treeWithMethod.getContainerProps()
    }
  }
...
function TreeDragLine({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  if (!tree || typeof tree !== 'object' || !('getDragLineStyle' in tree) || typeof (tree as Record<string, unknown>).getDragLineStyle !== 'function') {
    console.warn(
  ...
  const treeWithMethod = tree as { getDragLineStyle: () => Record<string, unknown> }
  const dragLine = treeWithMethod.getDragLineStyle()
  return (
    <div
      style={dragLine}
      className={cn(
